...
file <- file.path(script_dir, "statements-521.xlsx")
...
for (art_id in article_ids) {
  ...
  article_data <- subset(data, article_id == art_id)
...
alpha_results <- data.frame(article_id = integer(), title = character(), alpha = numeric(), stringsAsFactors = FALSE)
...
write.csv(alpha_results, "/Users/schaer/ownCloud/SMC-Experts/interrater-agreement-521.csv", row.names=FALSE, quote=TRUE) 
